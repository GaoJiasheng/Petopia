...
> **当前 1.0 生产实现（2026-07-25）**：
> - 角色使用 240 张身份精确的透明静态立绘，并为 12 个物种的
>   `var01 / stage C` 提供 48 条 8 帧手绘动作条；其他花色/成长档使用原立绘
>   的物种化动作编排，禁止互动期间换成错误花色或成年体。
> - 动作条、豪华度叠层和天气叠层以 lossless WebP 进入运行包，PNG 仅作为
>   美术母版；发布门禁逐像素验证两者一致。
> - Flutter 图片缓存上限为 96MB；动作只预热最常用的摸头条，播放结束立即
>   驱逐。设置提供“自动 / 精致 / 省电”，系统内存压力会对本次运行临时降级。
> - 下文 Spine/KTX2 和 SoC 自动分档属于后续演进目标，不得当作 1.0 已实现
>   能力。1.0 的真实性能验收以实际序列帧与真机 Instruments 数据为准。
...
- 首批地点背景板：**首只宠物毕业后 25 张主旅程**按需预取所需的背景板（可保守全带 MVP 地点 12 张，postcard 域）+ 12 物种×8 姿态贴层（`atlas_postcard_pose`）+ 6 滤镜 + 邮戳。
...
- **手动档**：设置页提供“自动 / 精致 / 省电”；自动档收到系统内存压力后
  临时收起非关键动态与动作预热，不写回用户选择。SoC/RAM/GPU 的完整自动
  判档与 thermal 回滞仍为 `[待细化]`。
...
- **明信片背景板（40 张，大且低命中）**：**逐张按需**——看某张明信片时加载对应 `pc_bg_*`，看完可驱逐。生成明信片（PostcardGenerator，spec-technical §3.5）时**提前预取下一站背景板**（主旅程 3–5 天、补完期 10–15 天均有离线窗口，§2.3 离线保证）。
