...
- `assets/runtime/postcards/backgrounds/*.webp`：40 张明信片背景运行件；
  `assets/art/postcards/backgrounds/` 下同名 PNG/JPG 保留为母图。
...
- 宠物透明边缘必须保持完整，不裁耳朵、尾巴、脚底柔和阴影或动作极值。
- 母版与运行件均进入发布门禁：240 张静态立绘、48 条动作图的每一帧、
  图鉴、访客与明信片姿态都会检查主体占比、顶部/左右安全区和底部柔和
  阴影余量；动作帧同时与 Stage C 静态原型比较尺寸与落地基线。
- 全量 alpha 边界审计：240 张静态立绘最小透明安全边距 `51 px`；48 条
  动作图逐帧最小安全边距 `54 px`，无贴边或截断。
...
- 图片缓存按逻辑短边分档：手机最多 `72 MiB / 240` 项，iPad 和大窗口最多
  `96 MiB / 320` 项；系统内存警告会取消预热并清理缓存，恢复两分钟后再按
  当前画质档逐步预热。
- iOS/Android 音频运行件统一使用原生支持的 M4A；OGG 和 WAV 继续保留为交付源，不进入首包。

## 当前规模

- 宠物运行件：288 个无损 WebP，36.61 MiB。
...
- `pubspec.yaml` 声明的 693 个发布素材合计 135.66 MiB；自动门禁上限为
  138 MiB。与本轮瘦身前约 147 MiB 的声明素材相比减少约 11.3 MiB。
- 2026-07-25 build 16 iOS device Release 展开包：198.3 MB
  （约 189.1 MiB）。
- 2026-07-27 build 17 iOS device Release 候选包：171.8 MB
  （文件合计 163.87 MiB）；本地 ZIP 压缩估算为 146.04 MiB。相比本轮前
  181.1 MB 的候选包减少 9.3 MB，且 380 组透明运行件继续通过逐像素
  一致性检查。
