...
abstract final class EnglishCopy {
  ...
  static const Map<String, String> _copy = <String, String>{
    ...
    '跟随设备语言，也可以在应用内固定选择。':
        'Follow your device, or choose a language just for Hearth & Tails.',
    '使用设备语言；暂不支持的语言会显示英语。':
    ...
    '只有在你主动开启后，App 才会请求本地通知权限。提醒仅用于明信片、老朋友回访和纪念日；可随时在设置中分类关闭，也不会使用广告追踪。':
        'Hearth & Tails asks for notification access only after you turn it on. Alerts are limited to postcards, returning friends, and anniversaries. Each category can be disabled anytime, and no ad tracking is used.',
    '存档备份': 'Save Backups',
    ...
    '当前版本不包含广告、第三方分析、跨 App 追踪、社交登录或联网内容服务。只有在你主动支持小院时，购买和恢复购买会由 Apple App Store 处理；暖绒小院不会接触支付卡或 Apple ID。':
        'Hearth & Tails contains no ads, third-party analytics, cross-app tracking, social login, or online content service. If you choose to support the garden, Apple handles purchases and restores; Hearth & Tails never receives card details or your Apple ID.',
    '抱抱它': 'Give a Hug',
    ...
    '只有在你主动选择“报告问题”时，App 才会打开邮件应用，并填入版本与运行状态摘要。发送前可自行查看和修改；内容不包含昵称、明信片正文、存档或设备标识。':
        'Only when you choose Report a Problem does Hearth & Tails open your mail app with a version and status summary. You can review or edit everything before sending. It excludes names, postcard text, saves, and device identifiers.',
    '明写': 'Open Goals',
